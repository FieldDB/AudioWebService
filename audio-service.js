...
app.post('/upload/extract/utterances', function(req, res) {
  ...
  if (req.files && req.files.videoFile) {
    ...
    // console.log(audioVideoFiles);
  } else if (req.body.data && req.body.data.indexOf("data") === 0) {
    var fileContents = req.body.data;

    var result = fs.writeFileSync("/Users/gina/fielddbhome/uploadinvestigation/contents_withoutbase64.mp3", fileContents.split('base64,')[1]);
    console.log("wrote file without base64 ", result);

    result = fs.writeFileSync("/Users/gina/fielddbhome/uploadinvestigation/contents_withbase64.mp3", fileContents);
    console.log("wrote file contents", result);

    var buf = new Buffer(fileContents, "base64");
    var wstream = fs.createWriteStream("/Users/gina/fielddbhome/uploadinvestigation/contents_withbase64_buffer.mp3");
    wstream.write(buf);
    wstream.end();

    buf = new Buffer(fileContents.split('base64,')[1], "base64");
    wstream = fs.createWriteStream("/Users/gina/fielddbhome/uploadinvestigation/contents_withoutbase64_buffer.mp3");
    wstream.write(buf);
    wstream.end();


    buf = new Buffer(fileContents.split('base64,')[1], "base64");
    wstream = fs.createWriteStream("/Users/gina/fielddbhome/uploadinvestigation/contents_withoutbase64_buffer_tostring.mp3");
    wstream.write(buf.toString());
    wstream.end();

    //http://stackoverflow.com/questions/6182315/how-to-do-base64-encoding-in-node-js/6182519#6182519

    //http://www.acnenomor.com/211698p1/save-mp3-as-ajax-post-data-with-node-js
    var mp3File = "/Users/gina/fielddbhome/uploadinvestigation/file_open_write_file_contents_withoutbase64_buffer_tostring.mp3";
    var mp3 = fileContents;
    ...
    audioVideoFiles.push({
      size: 37615,
      path: '/Users/gina/fielddbhome/uploadinvestigation/temp_buf.mp3',
      name: 'temp_buf.mp3',
      type: 'audio/mp3',
      mtime: '2014-12-22T10:45:38.614Z',
      fileBaseName: 'temp_buf',
      praatAudioExtension: '.mp3',
      dbname: 'hardcoded-firstcorpus'
    });
    // console.log(audioVideoFiles);
  } else {
    res.statusCode = 422;
