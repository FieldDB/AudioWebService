...
var praatCommand = os.platform() === "darwin" ? "/Applications/Praat.app/Contents/MacOS/Praat --run " : "praat --run ";

var TextGrid = function TextGrid(options) {
	this.audioFile = options ? options.audioFile : "";
	this.workingDir = options ? options.workingDir : "";
	this.script = options ? options.script : "";
	Object.call(this);
};
	...
	generate: {
		value: function() {
			...
			process.nextTick(function() {
				...
				if (self.script === "LongSound") {
					textGridCommand = praatCommand + __dirname.replace(/lib$/, "")
					...
					textGridCommand = praatCommand + __dirname.replace(/lib$/, "")
					+ "node_modules/praat-scripts/praat-script-syllable-nuclei-v2file.praat -20 4 0.4 0.1 no \""
					+ self.workingDir + "\"   \"" + self.audioFile + "\""; //+ " 2>&1 ";
				}
