...
var WEB_PLAYABLE_EXTENSION = ".mp3";
var CONVERT_NON_RAW_AUDIO = " -ar 22050 ";
var CONVERT_RAW_8K_AUDIO = " -f s16le -ar 16k ";
...
var generateTextGrid = function(localFileDetails, localRawDir, localDeferredPromise) {
	...
	new TextGrid({
		...
		.then(function(textGridResults) {
            ...
            fs.writeFile(localFileDetails.currentWorkingDir + "/" + localFileDetails.fileBaseName + ".json", JSON.stringify(textGridResults, null, 2), function(err) {
                if (err) {
                    console.log(new Date() + " failed to write textGridResults", err);
                }
                console.log(new Date() + " wrote textGridResults");
            });
...
var ensureWebPlayableAudioExists = function(localFileDetails, localRawDir) {
	var optionalFfmpegConvertParameters = "";
	var optionalFfmpegSecondConvertParameters = CONVERT_NON_RAW_AUDIO;
	if (localFileDetails.name.indexOf(".raw") === localFileDetails.name.length - 4) {
		optionalFfmpegConvertParameters = CONVERT_RAW_8K_AUDIO;
		optionalFfmpegSecondConvertParameters = "";
	}
	console.log(new Date() + " " + localFileDetails.fileBaseName + " Verifying web playable result.");
	if (fs.existsSync(localRawDir + "/" + localFileDetails.uploadFileId + WEB_PLAYABLE_EXTENSION)) {
		...
		localFileDetails.webResultInfo = updateSymlink(localRawDir + "/" + localFileDetails.uploadFileId + WEB_PLAYABLE_EXTENSION,
			localFileDetails.currentWorkingDir + "/" + localFileDetails.fileBaseName + WEB_PLAYABLE_EXTENSION);
	} else {
		audioResultCommand = "avconv -y " + optionalFfmpegConvertParameters + "  -i \"" + localFileDetails.currentWorkingDir + "/" + localFileDetails.name +
			"\" -ac 1 " + optionalFfmpegSecondConvertParameters + " \"" + localFileDetails.currentWorkingDir + "/" + localFileDetails.fileBaseName + WEB_PLAYABLE_EXTENSION + "\"";
		...
		shellPromises.execute(audioResultCommand)
			.then(function(results) {
				...
				fs.renameSync(localFileDetails.currentWorkingDir + "/" + localFileDetails.fileBaseName + WEB_PLAYABLE_EXTENSION, localRawDir + "/" + localFileDetails.uploadFileId + WEB_PLAYABLE_EXTENSION);
				localFileDetails.webResultInfo = updateSymlink(localRawDir + "/" + localFileDetails.uploadFileId + WEB_PLAYABLE_EXTENSION,
					localFileDetails.currentWorkingDir + "/" + localFileDetails.fileBaseName + WEB_PLAYABLE_EXTENSION);
			})
			.fail(function(reason) {
...
exports.createAudioFromUpload = function(filesWithDetails, dbname, rawDir, destinationDir) {
	...
	process.nextTick(function() {
		for (var fileindex = 0; fileindex < resultDetails.length; fileindex++) {
			...
			if (!resultDetails[fileindex].name || resultDetails[fileindex].name === "blob" || resultDetails[fileindex].name.indexOf(".") === -1) {
				console.log("file name was missing, it was a blob");
				resultDetails[fileindex].name = "file_" + Date.now() + "." + resultDetails[fileindex].type.split("/")[1];
			}
			...
			(function(fileDetails, boundindex) {
				...
				checksum.file(fileDetails.path, function(error, checksumResults) {
					...
					try {
						mkdirp.sync(fileDetails.currentWorkingDir);
					} catch (e) {
						if (e.errno !== 47) {
							console.log(e);
						} else {
					...
					console.log("audioResultIsNotEmpty", audioResultIsNotEmpty);
					if (audioResultExists && audioResultIsNotEmpty.size > 10) {
						...
						var optionalFfmpegConvertParameters = "";
						var optionalFfmpegSecondConvertParameters = CONVERT_NON_RAW_AUDIO;
						if (fileDetails.name.indexOf(".raw") === fileDetails.name.length - 4) {
							optionalFfmpegConvertParameters = CONVERT_RAW_8K_AUDIO;
							optionalFfmpegSecondConvertParameters = "";
						}
						audioResultCommand = "avconv -y " + optionalFfmpegConvertParameters + " -i \"" + fileDetails.currentWorkingDir + "/" + fileDetails.name +
							"\" -ac 1 " + optionalFfmpegSecondConvertParameters + " \"" + rawDir + "/" + fileDetails.uploadFileId + fileDetails.praatAudioExtension + "\"";
