...
var generateTextGrid = function(localFileDetails, localDeferredPromise) {
	...
	new TextGrid({
		...
		.then(function(textGridResults) {
			...
			// fs.renameSync(localFileDetails.currentWorkingDir + "/" + localFileDetails.fileBaseName + ".TextGrid", localFileDetails.currentWorkingDir + "/" + localFileDetails.fileBaseName + ".TextGrid");
		...
		for (var fileindex = 0; fileindex < resultDetails.length; fileindex++) {
			...
			(function(fileDetails, boundindex) {
				...
				checksum.file(fileDetails.path, function(error, checksumResults) {
					...
					if (fs.existsSync(rawDir + "/" + fileDetails.uploadFileId + fileDetails.praatAudioExtension)) {
						...
						generateTextGrid(fileDetails, individualDeferred);
					} else {
						audioResultCommand = "ffmpeg -y -i \"" + fileDetails.currentWorkingDir + "/" + fileDetails.name +
							"\" -ac 1 -ar 22050 \"" + fileDetails.currentWorkingDir + "/" + fileDetails.fileBaseName + fileDetails.praatAudioExtension + "\"";
						...
						shellPromises.execute(audioResultCommand)
							.then(function(results) {
								...
								fs.renameSync(fileDetails.currentWorkingDir + "/" + fileDetails.fileBaseName + fileDetails.praatAudioExtension, rawDir + "/" + fileDetails.uploadFileId + fileDetails.praatAudioExtension);
								fileDetails.resultInfo = updateSymlink(rawDir + "/" + fileDetails.uploadFileId + fileDetails.praatAudioExtension,
									fileDetails.currentWorkingDir + "/" + fileDetails.fileBaseName + fileDetails.praatAudioExtension);
								generateTextGrid(fileDetails, individualDeferred);
							})
				...
				Q.allSettled(promises)
					.then(function(results) {
						results.forEach(function(result) {
							...
							ensureWebPlayableAudioExists(result.value, rawDir);
						});
						deferred.resolve(resultDetails);
					});
			}
